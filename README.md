...
1. When you encrypt a file, SecureVault generates a random 16-byte salt and 12-byte IV.
2. Your password plus the salt are run through PBKDF2 (100,000 iterations) to derive a 256-bit AES key. The raw password is never stored or used directly as a key.
3. The file is encrypted with AES/GCM/NoPadding using that key and IV. GCM appends a 128-bit authentication tag to the ciphertext.
4. The output file (`<filename>.vault`) stores `[salt][iv][ciphertext+tag]` — everything needed to decrypt, except the password, which only you know.
5. On decryption, SecureVault re-derives the key from the password and the stored salt, then decrypts. If the password is wrong or the file was altered, the GCM authentication check fails and decryption is rejected — this is what gives the tool its integrity guarantee.
